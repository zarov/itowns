...
describe('Entwine Point Tile', function () {
    ...
    describe('Node', function () {
        ...
        it('gets the correct parent of a node', () => {
            let node = new EntwinePointTileNode(4, 0, 5, 14, layer, 500);
            let parent = root.findParent(node.depth - 1, Math.floor(node.x / 2), Math.floor(node.y / 2), Math.floor(node.z / 2));
            assert.deepStrictEqual(parent, root.children[1].children[0].children[0]);

            node = new EntwinePointTileNode(3, 1, 5, 5, layer, 1000);
            parent = root.findParent(node.depth - 1, Math.floor(node.x / 2), Math.floor(node.y / 2), Math.floor(node.z / 2));
            assert.deepStrictEqual(parent, root.children[2].children[0]);

            node = new EntwinePointTileNode(5, 9, 5, 5, layer, 250);
            parent = root.findParent(node.depth - 1, Math.floor(node.x / 2), Math.floor(node.y / 2), Math.floor(node.z / 2));
            assert.deepStrictEqual(parent, undefined);
        });
